...
function clearFieldFromClasses(){
  var $table = document.getElementById('game-table');

  for (var i = 0; i < FIELD_SIZE_X; i ++){
    for (var j = 0; j < FIELD_SIZE_Y; j++){
      if ($table.children[snakeCoordX].children[snakeCoordY].classList.contains("snake-unit")){
        $table.children[snakeCoordX].children[snakeCoordY].classList.remove("snake-unit");
      }
      if ($table.children[snakeCoordX].children[snakeCoordY].classList.contains("food-unit")){
        $table.children[snakeCoordX].children[snakeCoordY].classList.remove("food-unit");
      }
    }
  }
}
